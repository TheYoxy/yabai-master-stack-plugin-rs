...
use crate::yabai::config::get_state_path;
use crate::yabai::spaces::Space;
...
impl StateForSpace for State {
    fn get_space(&self, space: &Space) -> color_eyre::Result<&usize> {
        ...
        self.get(&space.id)
            .ok_or(eyre!("Unable to get the space {id}", id = space.id))
    }

    fn get_space_mut(&mut self, space: &Space) -> color_eyre::Result<&mut usize> {
        use color_eyre::eyre::eyre;
        self.get_mut(&space.id)
            .ok_or(eyre!("Unable to get the space {id}", id = space.id))
    }
}
...
pub fn write_state(state: &State) -> color_eyre::Result<()> {
    ...
    debug!(
        "Writing state to {state_file_path:?}",
        state_file_path = state_file_path.yellow()
    );
    let file = std::fs::File::create(&state_file_path)?;
    serde_json::to_writer(file, state)?;
    debug!(
        "State written to {state_file_path:?}",
        state_file_path = state_file_path.yellow()
    );
...
pub fn read_state() -> color_eyre::Result<State> {
    ...
    let state_file_path = get_state_path()?;
    debug!("Looking for state file at: {:?}", state_file_path);
    #[cfg(debug_assertions)]
    {
        debug!("Removing state file for testing purposes");
    ...
    if exists {
        ...
        for space in &spaces {
            debug!(
                "Adding space {space_id} to the state",
                space_id = space.id.blue()
            );
            state.entry(space.id).or_insert(1);
        }
        write_state(&state)?;
