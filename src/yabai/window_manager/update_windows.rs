use crate::yabai::window_manager::yabai::YabaiCommand;
use crate::yabai::{
                ...
                if num_windows > 1 {
                    ...
                    while cur_num_master_windows > target_num_master_windows {
                        ...
                        if let Some(master_window) = master_window {
                            info!("Moving master window {master_window} to stack.");
                            self.move_window_to_stack(&master_window)?;
                            info!("Moved window {master_window} to stack.");
                        }
                    ...
                    while !middle_windows.is_empty() {
                        ...
                        if let Some(middle_window) = middle_window {
                            ...
                            if cur_num_master_windows < target_num_master_windows {
                                info!("Moving middle window {middle_window} to master.");
                                self.move_window_to_master(middle_window)?;
                                cur_num_master_windows += 1;
                            } else {
                                info!("Moving middle window {middle_window} to stack.");
                                self.move_window_to_stack(middle_window)?;
                            }
                            dead_lock += 1;
                        } else {
                            info!("Unable to get middle window");
                            dead_lock += 1;
                        }
                    ...
                    while cur_num_master_windows < target_num_master_windows {
                        info!("Not enough master windows ({cur_num_master_windows}/{target_num_master_windows})");
                    ...
                    match result {
                        ...
                        LayoutValidity::Invalid(reason) => {
                            bail!("update_windows() ended with ans invalid layout; reason {reason}")
                        }
                    }
