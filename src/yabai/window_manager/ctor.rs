...
use crate::{
  ...
  yabai::{
    ...
    window::{get_windows, SplitType, Window},
    window_manager::yabai::{get_yabai_config, YabaiCommand, YabaiConfig},
  },
};
...
impl WindowsManager {
  ...
  fn get_bottom_window(&self, windows: Vec<Window>) -> Option<Window> {
    if windows.is_empty() {
  ...
  pub(crate) fn is_top_window(&self, master_windows: Vec<Window>, focused_window: &Window) -> bool {
    let top_window = self.get_top_window(master_windows);
  ...
  pub(crate) fn is_bottom_window(&self, master_windows: Vec<Window>, focused_window: &Window) -> bool {
    let bottom_window = self.get_bottom_window(master_windows);
  ...
  pub(crate) fn is_stack_window(&self, window: &Window) -> Result<bool> {
    ...
    trace!(
      ...
      result = if result { result.color(AnsiColors::Green) } else { result.color(AnsiColors::Red) }
    );
    Ok(result)
  }
